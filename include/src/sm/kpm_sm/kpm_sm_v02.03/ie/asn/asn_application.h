 ...
 */
asn_enc_rval_t asn_encode_kpm_v2_03_to_buffer(
...
/*
 * A variant of asn_encode_kpm_v2_03_to_buffer() with automatically allocated buffer.
 * RETURN VALUES:
 * On success, returns a newly allocated (.buffer) containing the whole message.
 * The message size is returned in (.result.encoded).
 * On failure:
 *  (.buffer) is NULL,
 *  (.result.encoded) as in asn_encode_kpm_v2_03_to_buffer(),
 *  The errno codes as in asn_encode_kpm_v2_03_to_buffer(), plus the following:
 *      ENOMEM: Memory allocation failed due to system or internal limits.
 * The user is responsible for freeing the (.buffer).
 */
typedef struct asn_encode_kpm_v2_03_to_new_buffer_result_s {
    void *buffer;   /* NULL if failed to encode. */
    asn_enc_rval_t result;
} asn_encode_kpm_v2_03_to_new_buffer_result_t;
asn_encode_kpm_v2_03_to_new_buffer_result_t asn_encode_kpm_v2_03_to_new_buffer(
    const asn_codec_ctx_t *opt_codec_parameters, /* See asn_codecs.h */
    enum asn_transfer_syntax,
    const struct asn_TYPE_descriptor_s *type_to_encode,
 ...
 */
asn_enc_rval_t asn_encode_kpm_v2_03(
    const asn_codec_ctx_t *opt_codec_parameters, /* See asn_codecs.h */
    enum asn_transfer_syntax,
    const struct asn_TYPE_descriptor_s *type_to_encode,
    const void *structure_to_encode,
 ...
 */
asn_dec_rval_t asn_decode_kpm_v2_03(
